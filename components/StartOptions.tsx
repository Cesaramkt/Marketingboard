...
export const StartOptions: React.FC<StartOptionsProps> = ({ onSelect }) => {
    ...
    const OptionCard: React.FC<{title: string, description: string, icon: React.ReactNode, onClick: () => void}> = ({ title, description, icon, onClick }) => (
        <div 
            onClick={onClick}
            className="bg-slate-800/50 backdrop-blur-sm p-8 rounded-2xl shadow-lg hover:shadow-purple-500/10 border border-slate-700 hover:border-purple-500 transition-all duration-300 cursor-pointer transform hover:-translate-y-1"
        >
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <h2 className="text-4xl font-bold text-center text-slate-100 mb-2 font-['Playfair_Display',_serif]">Como você quer começar?</h2>
            <p className="text-center text-lg text-slate-400 mb-10 max-w-2xl mx-auto">Escolha uma opção para iniciar a criação do seu brandboard.</p>
