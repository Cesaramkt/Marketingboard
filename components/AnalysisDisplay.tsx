...
export const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({ analysisText, sources, onConfirm, onBack }) => {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm p-6 sm:p-10 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700">
                <h1 className="text-3xl font-bold text-center text-gray-900 dark:text-slate-100 mb-2 font-['Playfair_Display',_serif]">Análise Profunda da Marca</h1>
                <p className="text-center text-gray-600 dark:text-slate-400 mb-8">Nossa IA realizou uma pesquisa sobre a empresa. Revise o diagnóstico abaixo. Ele será a base para criar todo o marketingboard.</p>
                ...
                <div className="space-y-6">
                    {analysisSections.map((section, index) => {
                         ...
                         return (
                            <div key={index} className="bg-gray-50 dark:bg-slate-800 p-6 rounded-xl border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 transition-all hover:border-purple-400/50 dark:hover:border-purple-500/30">
                                <h2 className="text-xl font-bold text-purple-600 dark:text-purple-400 mb-4">{section.title}</h2>
                                <div className="prose prose-slate dark:prose-invert max-w-none text-gray-700 dark:text-slate-300">
                                    {paragraphs}
                                    {listItems.length > 0 && (
                ...
                {sources && sources.length > 0 && (
                    <div className="mt-8">
                        <h3 className="font-semibold text-gray-800 dark:text-slate-200 text-lg">Fontes Utilizadas na Análise:</h3>
                        <ul className="mt-2 text-sm space-y-1">
                            {sources.map((source, index) => (
                                <li key={index} className="flex items-start">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2 text-purple-500 dark:text-purple-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                                    </svg>
                                    <a href={source.uri} target="_blank" rel="noopener noreferrer" className="text-purple-600 dark:text-purple-400 hover:underline break-all">{source.title}</a>
                                </li>
                            ))}
                        </ul>
                    </div>
